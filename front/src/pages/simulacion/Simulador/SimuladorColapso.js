...
const SimuladorColapso = () => {
	const [fechaInicioSimulacion, setFechaInicioSimulacion] = useState(""); // la fecha que envias
	const [horaInicioSimulacion, setHoraInicioSimulacion] = useState("00:00"); // la hora que envias (formato HH:mm)
	...
	const [simulationStatus, setSimulationStatus] = useState('Simulación semanal activa');
	...
	// ===================== MODO SEMANAL (ÚNICO MODO) ====================
	// El simulador opera exclusivamente en modo semanal
	const modoPanel = 'semanal';
	
	// Limpiar claves antiguas del localStorage al montar
	useEffect(() => {
		localStorage.setItem('modoPanel', 'semanal');
	...
	// Variables para mantener compatibilidad con componentes existentes
	const cargandoRutas = false;
	...
	useEffect(() => {
		...
		airports.forEach(airport => {
			...
			if (saturacion >= UMBRAL_SATURACION_AEROPUERTO) {
				const alertaKey = `saturacion-${airport.code}-${Math.floor(saturacion / 5) * 5}`;
		...
		if (alertasCriticas.length >= 5) {
			const alertaKey = `colapso-sistema-${Math.floor(Date.now() / 60000)}`;
			if (!alertasVistaRef.current.has(alertaKey)) {
				alertasVistaRef.current.add(alertaKey);
				nuevasAlertas.push({
					id: alertaKey,
	...
	// 🆕 FUNCIÓN: Buscar destino final de un pedido usando vuelosEnMovimiento
	const buscarDestinoFinalPedido = useCallback((idPedido, vuelosParaBuscar = []) => {
		...
		// Buscar en vuelosEnMovimiento pasados como parámetro
	...
	const handleIniciarSimulacion = async () => {
		...
		try {
			...
			// Llamar al endpoint REST para iniciar simulacion diaria
			console.log('Llamando al backend para iniciar simulacion diaria...');
	...
	/**
	 * Iniciar simulacion diaria con WebSocket STOMP
	 */
	const iniciarSimulacionWebSocketStomp = useCallback(async () => {
		...
		try {
			console.log('Iniciando simulacion diaria con WebSocket STOMP...');

			// LIMPIAR VUELOS ANTERIORES antes de iniciar nueva simulacion
			console.log('Limpiando vuelos anteriores...');
			setFlights([]);
			setFlightsInAir(0);
			setProgresoAG(null);
			setMensajesSimulacion([]);
			contadorVuelosRef.current = 0; // Resetear contador de IDs unicos
			...
			agregarMensaje(`Iniciando simulacion diaria para ${fechaInicioSimulacion} a las ${horaInicioSimulacion}`, 'info');
					...
					<div className="sidebar-content" style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', gap: '4px' }}>
						...
						<Box sx={{
							...
							<Box sx={{
								...
								{sidebarTab === 'orders' && (
									<Box>
										{/* ===== MODO SEMANAL: Mostrar pedidos de vuelos ===== */}
										<>
											{(() => {
											const list = [];
...
{/* Botón de control - abre el panel de simulación semanal */}
										<IconButton
											ref={(el) => {
										...
										>
											<HiAdjustments
												sx={{
													color: "#2c4a6b", fontSize: 30, stroke: "white", strokeWidth: 0.8
												}}
											/>
							</IconButton>
