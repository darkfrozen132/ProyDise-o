// GENETICO - Versión Standalone
// Algoritmo Genético con Decoder Greedy para optimización de rutas logísticas
// Ejecutar con: java genetico_standalone.java
// Los archivos Aeropuertos.txt, PlanesDeVuelo.txt y Pedidos.txt deben estar en la misma carpeta
...
public class genetico_standalone {
    ...
    static Precomp precompute(World W, int horizonDays){
        ...
        for (int fi=0; fi<n; fi++){
            ...
            // Validar que existan ambos aeropuertos
            if (aO == null || aD == null) {
                System.err.println("⚠ Advertencia: Vuelo " + f.orig + "->" + f.dest + " tiene aeropuerto(s) no encontrado(s)");
                continue;
            }
