...
public class GeneradorRutasSimple {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            String archivoAeropuertos = args.length > 0 ? args[0] : "datos/aeropuertos_simple.csv";
            String archivoVuelos = args.length > 1 ? args[1] : "datos/vuelos_simple.csv";
            String archivoPedidos = args.length > 2 ? args[2] : "datos/pedidos.csv";
    ...
    private static void cargarAeropuertos(String archivo) throws IOException {
        ...
        for (int i = 1; i < lineas.size(); i++) {
            ...
            if (datos.length >= 9) {
                try {
                    String codigo = datos[0].trim(); // CodigoICAO (primera columna)
                    String ciudad = datos[1].trim(); // Ciudad
                    String pais = datos[2].trim(); // Pais
                    int huso = Integer.parseInt(datos[4].trim()); // HusoHorario
                    int capacidad = Integer.parseInt(datos[5].trim()); // Capacidad
                    double lat = Double.parseDouble(datos[6].trim()); // Latitud
                    double lon = Double.parseDouble(datos[7].trim()); // Longitud
